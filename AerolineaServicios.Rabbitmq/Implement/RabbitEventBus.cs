...
namespace AerolineaServicios.Rabbitmq.Implement
{
    ...
    public class RabbitEventBus: IRabbitEventBus
    {
        ...
        {
            var factory = new ConnectionFactory() { HostName = "13.84.206.11",UserName = "user",Password="JYUJRArabbit123"};
            ...
            where TH : IEventoManejador<T>
        {
            ...
            {
                throw new ArgumentException($"EL manejador {manejadorEventoTipo.Name} fue resgistardo anteriormente por {eventoNombre}");
            }
            _manejadores[eventoNombre].Add(manejadorEventoTipo);

            var factory = new ConnectionFactory() { HostName = "13.84.206.11", UserName = "user", Password = "JYUJRArabbit123" , DispatchConsumersAsync=true };
